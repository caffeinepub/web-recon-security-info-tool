...
export async function fetchSubdomains(domain: string): Promise<SubdomainResult[]> {
  try {
    ...
    try {
      ...
      const res = await fetch(withProxy(url), { signal: AbortSignal.timeout(15000) });
...
export async function fetchHostingInfo(domain: string): Promise<HostingInfo | null> {
  try {
    // First resolve domain to IP via HackerTarget DNS
    let ip = domain;
    try {
      const dnsUrl = `${HT_BASE}/dnslookup/?q=${encodeURIComponent(domain)}`;
      const dnsRes = await fetch(withProxy(dnsUrl), { signal: AbortSignal.timeout(10000) });
...
export async function fetchWhoisData(domain: string): Promise<WhoisData | null> {
  try {
    ...
    const creationDate = extract([/Creation Date:\s*(.+)/i, /Created:\s*(.+)/i, /created:\s*(.+)/i]);
    const expiryDate = extract([/Registry Expiry Date:\s*(.+)/i, /Expiry Date:\s*(.+)/i, /Expiration Date:\s*(.+)/i, /expires:\s*(.+)/i]);
    const updatedDate = extract([/Updated Date:\s*(.+)/i, /Last Modified:\s*(.+)/i, /last-update:\s*(.+)/i]);
    const nameServers = extractAll(/Name Server:\s*(.+)/i);
    const status = extract([/Domain Status:\s*(.+)/i, /Status:\s*(.+)/i]);
    const registrant = extract([/Registrant Organization:\s*(.+)/i, /Registrant Name:\s*(.+)/i, /org:\s*(.+)/i]);
...
export async function fetchSslCertInfo(domain: string): Promise<SslCertInfo | null> {
  try {
    ...
    const sorted = [...data].sort((a, b) =>
      new Date(b.not_before || 0).getTime() - new Date(a.not_before || 0).getTime()
    );
    const cert = sorted[0];
...
export async function fetchHttpHeaders(domain: string): Promise<HttpHeaderInfo[]> {
  try {
...
export async function runFullScan(domain: string): Promise<ReconData> {
  ...
  const [subdomains, hostingInfo, whoisData, dnsRecords, sslCertInfo, httpHeaders] =
  ...
  const getResult = <T>(result: PromiseSettledResult<T>, key: string, fallback: T): T => {
...
export function serializeReconData(data: ReconData) {
  return {
    subdomains: data.subdomains.map(s => s.ip ? `${s.subdomain} (${s.ip})` : s.subdomain),
