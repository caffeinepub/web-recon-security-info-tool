import { FileText, Calendar, User, Server } from 'lucide-react';
  ...
  return (
    <div className="flex items-start gap-3 px-4 py-3 rounded bg-cyber-surface1 border border-border/50">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="font-mono text-sm text-foreground break-all">{value || 'N/A'}</div>
      </div>
    </div>
  );
}
  ...
  if (!whoisData) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        ...
        <p className="font-mono text-xs mt-1 opacity-60">API rate limit may have been reached</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <InfoRow label="Registrar" value={whoisData.registrar} icon={Building2Placeholder} />
        <InfoRow label="Registrant" value={whoisData.registrant} icon={User} />
        <InfoRow label="Status" value={whoisData.status} icon={FileText} />
        <InfoRow label="Created" value={whoisData.creationDate} icon={Calendar} />
        <InfoRow label="Expires" value={whoisData.expiryDate} icon={Calendar} />
        <InfoRow label="Updated" value={whoisData.updatedDate} icon={Calendar} />
        <div className="sm:col-span-2">
          <div className="flex items-start gap-3 px-4 py-3 rounded bg-cyber-surface1 border border-border/50">
            <Server className="w-4 h-4 text-cyber-cyan mt-0.5 flex-shrink-0" />
            <div className="min-w-0 flex-1">
              <div className="text-xs font-mono text-muted-foreground uppercase tracking-wider mb-1">Name Servers</div>
              <div className="space-y-1">
                {whoisData.nameServers.map((ns, i) => (
                  <div key={i} className="font-mono text-sm text-foreground">{ns}</div>
                ))}
              </div>
            </div>
          </div>
