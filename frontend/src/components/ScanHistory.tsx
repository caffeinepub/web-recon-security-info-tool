import { History, Clock, ChevronRight, Loader2, RefreshCw, Trash2 } from 'lucide-react';
import { ScrollArea } from '@/components/ui/scroll-area';
import { useGetScanHistory } from '../hooks/useQueries';
import { useActor } from '../hooks/useActor';
import { deserializeReconData, type ReconData } from '../services/reconApi';
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className={`font-mono text-xs font-medium truncate ${
                        activeDomain === domain ? 'text-cyber-cyan' : 'text-foreground'
                      }`}>
                        {domain}
                      </div>
                    ...
                    <ChevronRight className={`w-3.5 h-3.5 flex-shrink-0 transition-colors ${
                      activeDomain === domain ? 'text-cyber-cyan' : 'text-muted-foreground/40 group-hover:text-muted-foreground'
                    }`} />
                  </button>
                ))}
            </div>
