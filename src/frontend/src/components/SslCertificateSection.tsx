...
export function SslCertificateSection({
  ...
  const isExpired =
    sslCertInfo.notAfter !== "Unknown" &&
    new Date(sslCertInfo.notAfter) < new Date();
  const isExpiringSoon =
    !isExpired &&
    sslCertInfo.notAfter !== "Unknown" &&
    new Date(sslCertInfo.notAfter) <
    ...
    <div className="space-y-4">
      {isExpired && (
        <div className="flex items-center gap-2 px-4 py-3 rounded bg-cyber-danger/10 border border-cyber-danger/30">
          ...
          <span className="font-mono text-sm text-cyber-danger">
      ...
      {isExpiringSoon && (
        <div className="flex items-center gap-2 px-4 py-3 rounded bg-cyber-warning/10 border border-cyber-warning/30">
          ...
          <span className="font-mono text-sm text-cyber-warning">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="sm:col-span-2">
          <InfoRow
            ...
            value={sslCertInfo.serialNumber}
            icon={Shield}
          />
        </div>
      </div>
        ...
        <div>
          ...
          <ScrollArea className="h-36">
            <div className="space-y-1 pr-3">
              {sslCertInfo.sans.map((san) => (
                <div
                  key={san}
                  className="flex items-center gap-2 px-3 py-1.5 rounded bg-cyber-surface1 border border-border/30"
                >
                  <span className="text-cyber-cyan-dim font-mono text-xs">
                    ›
                  </span>
