    ...
    <div>
      <div className="flex flex-wrap gap-2 mb-4">
        {Object.keys(grouped).map((type) => (
          <Badge
            key={type}
            className={`font-mono text-xs ${TYPE_COLORS[type] || "bg-muted text-muted-foreground"}`}
          >
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border/50 hover:bg-transparent">
              <TableHead className="font-mono text-xs text-muted-foreground uppercase tracking-wider w-20">
                Type
              </TableHead>
          ...
          <TableBody>
            {dnsRecords.map((record, i) => (
              <TableRow
                ...
                className="border-border/30 hover:bg-cyber-surface1/50"
              >
                <TableCell className="py-2">
                  <Badge
                    className={`font-mono text-xs ${TYPE_COLORS[record.type] || "bg-muted text-muted-foreground"}`}
                  >
                    {record.type}
                  </Badge>
                </TableCell>
